...
As always, we will need to bring in our text data prior to running the analysis.  We will be using the same DFM object as we did in the LDA tutorial.
...
Topics 13 and 10 seem to be outliers in terms of coherence and exclusivity, respectively.  We can take a closer look at the probable terms for these topics to get a sense of the problem.
...
There is also a more general exploration tool called `stmBrowser`. You can download the package using the following code (be sure to download all dependencies).  
...
With this tool you can explore bivariate (even multivariate) relationships between topics and easily read examples of documents with various topic probabilities. You can also observe the relationship between covariates and topics.  For instance, we could set the color of the labels in the graph to "source".
