...
2. Calculate bais statistics on corpus/dfm objects, including ttr, keyness and sentiment.
3. Plot these statistics using R, including how they change over time.

## Case Study: Ukraine in the media, 2022 and 2023

Last week we used the Guardian's web API to acquire a corpus of articles from the first month of this year on Ukraine. We also have a similar corpus of articles from January 2022 with which to compare this corpus. We might be interested in learning how coverage of the conflict has changed over this period: we discovered last week, for instance, that the term "Nato" was prominent in 2022, but not in 2023 (or at least, not in relation to Ukraine), while the word "give" has become prominent in 2023 in a way that it wasn't last year.

Today we're going to review the pre-processing steps we took last week to produce two comparable corpuses and dfms, and then perform some textual analysis to see if we can identify any further findings from the data.
